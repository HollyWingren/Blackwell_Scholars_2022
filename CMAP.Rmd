...
#new block
```{r}
prec_ras = raster(apply(matrix(prec_mat[,18], nrow=5, ncol=6, byrow=TRUE), 2, rev), xmn=-73.75, xmx=-61.25, ymn=1.25, ymx=11.25)
plot(prec_ras)

#prec.ras = raster(prec_mat[,c(1:2,3)])
obs.discrete = discretizeRaster(prec_ras, cellsize=480, psf = "gau", sigma = 2)
deconv_precip = deconvPointVgm(obs.discrete, model = "Sph", maxIter = 100, fig=TRUE, ngroup = 24, longlat = TRUE)
```

#Point Variogram
```{r}
for(m in 1:24) {
  prec.ras = raster(prec_mat[,c(1:2,m+2)])
  obs.discrete = discretizeRaster(prec.ras, cellsize=480, psf = "gau", sigma=2)
    if(m %in% c(3, 4, 6, 10, 14, 19, 20, 23, 24)) {
        deconv_precip = deconvPointVgm(obs.discrete, model = "Gau", maxIter = 100, fig=TRUE, ngroup = 24, longlat = TRUE)
        my_title = paste0("Gaussian Deconvolution for Month ", m)
    } else if(m %in% c(1, 2, 5, 7, 8, 9, 11, 12, 13, 15, 16, 17)) {
        deconv_precip = deconvPointVgm(obs.discrete, model = "Sph", maxIter = 100, fig=TRUE, ngroup = 24, longlat = TRUE)
        my_title = paste0("Spherical Deconvolution for Month ", m)
    } else {
        deconv_precip = deconvPointVgm(obs.discrete, model = "Exp", maxIter = 100, fig=TRUE, ngroup = 24, longlat = TRUE)
        my_title = paste0("Exponential Deconvolution for Month ", m)
    }
...
#deconvPointVgm(obs.discrete, model = "Sph", maxIter = 100, fig=TRUE, ngroup = 24, longlat = TRUE)
