...
#ncpath <- "/Home/github/Blackwell_Scholars_2022/"
...
for(m in 1:24) {
  covpars<-deconv_precip[[m]]$pointVariogram
  outpars<-output.control()
  krigepars<-krige.control(type.krige = "ok", trend.d = "1st", trend.l = "1st",
            obj.model = NULL, cov.model = "spherical", cov.pars = c(covpars$psill,covpars$range))
  
  image(krige.conv(geo_precip_list[[m]], coords=geo_precip_list[[m]]$coords, data=geo_precip_list[[m]]$data,
           locations = new_lonlat, krige = krigepars, output=outpars))
}
```